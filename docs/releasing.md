...
Releases are driven by the root `VERSION` file. The current version is `0.1.0`, published as a regular release. Increment the version for each subsequent release; never replace an existing published version.
